...
double bernstein(double x, double xmin, double xmax, DoubleArray coefs, int nCoefs)
{
   ...
   for (int i = 1; i < degree; i++) {
      result = (result + t * RooFit::Detail::MathFuncs::binomial(degree, i) * coefs[i]) * s;
      t *= xScaled;
   }
...
double bernsteinIntegral(double xlo, double xhi, double xmin, double xmax, DoubleArray coefs, int nCoefs)
{
   ...
   for (int i = 0; i <= degree; ++i) {
      ...
      for (int j = i; j <= degree; ++j) { // power basisŧ
         double binCoefs = RooFit::Detail::MathFuncs::binomial(degree, j) * RooFit::Detail::MathFuncs::binomial(j, i);
